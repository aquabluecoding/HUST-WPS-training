...
namespace TinySTL
{
	...
	bool KDeque<T, Alloc>::back_full()const
	{
		return map_[mapSize_ - 1] && (map_[mapSize_]) == end().cur_;
	}
	template<class T, class Alloc>
	bool KDeque<T, Alloc>::front_full()const
	{
		return map_[0] && map_[0] == begin().cur_;
	}
	template<class T, class Alloc>
	void KDeque<T, Alloc>::deque_aux(size_t n, const value_type& val, std::true_type)
	{
		int i = 0;
		for (; i != n / 2; ++i)
			(*this).push_front(val);
	...
	void KDeque<T, Alloc>::deque_aux(Iterator first, Iterator last, std::false_type)
	{
	...
	template<class T, class Alloc>
	T *KDeque<T, Alloc>::getANewBuck()
	{
		return dataAllocator::allocate(getBuckSize());
	}
	template<class T, class Alloc>
	T** KDeque<T, Alloc>::getANewMap(const size_t size)
	{
		T **map = new T*[size];
	...
	template<class T, class Alloc>
	size_t KDeque<T, Alloc>::getNewMapSize(const size_t size)
	{
		return (size == 0 ? 2 : size * 2);
	}
	template<class T, class Alloc>
	size_t KDeque<T, Alloc>::getBuckSize()const
	{
		return (size_t)EBucksSize::BUCKSIZE;
	}
	template<class T, class Alloc>
	void KDeque<T, Alloc>::clear()
	{
	...
	template<class T, class Alloc>
	typename KDeque<T, Alloc>::reference KDeque<T, Alloc>::operator[] (size_type n)
	{
		return *(begin() + n);
	}
	template<class T, class Alloc>
	typename KDeque<T, Alloc>::reference KDeque<T, Alloc>::front()
	{
		return *begin();
	}
	template<class T, class Alloc>
	typename KDeque<T, Alloc>::reference KDeque<T, Alloc>::back()
	{
		return *(end() - 1);
	}
	//由于const迭代器的设计失误故以下三个const函数会丧失const特性
	template<class T, class Alloc>
	typename KDeque<T, Alloc>::const_reference KDeque<T, Alloc>::operator[] (size_type n) const
	{
		return *(begin() + n);
	}
	template<class T, class Alloc>
	typename KDeque<T, Alloc>::const_reference KDeque<T, Alloc>::front() const
	{
		return *begin();
	}
	template<class T, class Alloc>
	typename KDeque<T, Alloc>::const_reference KDeque<T, Alloc>::back() const
	{
		return *(end() - 1);
	}
	template<class T, class Alloc>
	typename KDeque<T, Alloc>::iterator KDeque<T, Alloc>::begin(){ return beg_; }
	template<class T, class Alloc>
	typename KDeque<T, Alloc>::iterator KDeque<T, Alloc>::end(){ return end_; }
	//迭代器设计失误，会使下面两个函数丧失const特性，暂时没有效解决办法故只能先这样妥协
	template<class T, class Alloc>
	typename KDeque<T, Alloc>::iterator KDeque<T, Alloc>::begin()const{ return beg_; }
	template<class T, class Alloc>
	typename KDeque<T, Alloc>::iterator KDeque<T, Alloc>::end()const{ return end_; }
	template<class T, class Alloc>
	KDeque<T, Alloc>::~KDeque()
	{
		for (int i = 0; i != mapSize_; ++i)
		{
			for (auto p = map_[i] + 0; !p && p != map_[i] + getBuckSize(); ++p)
	...
	KDeque<T, Alloc>::KDeque()
		:mapSize_(0), map_(0){}
	template<class T, class Alloc>
	KDeque<T, Alloc>::KDeque(size_type n, const value_type& val = value_type())
	{
		KDeque();
	...
	KDeque<T, Alloc>::KDeque(InputIterator first, InputIterator last)
	{
		KDeque();
		deque_aux(first, last, typename std::is_integral<InputIterator>::type());
	}
	template<class T, class Alloc>
	KDeque<T, Alloc>::KDeque(const KDeque& x)
	{
		mapSize_ = x.mapSize_;
		map_ = getANewMap(mapSize_);
	...
	template<class T, class Alloc>
	void KDeque<T, Alloc>::reallocateAndCopy()
	{
	...
	template<class T, class Alloc>
	void KDeque<T, Alloc>::push_back(const value_type& val)
	{
		if (empty())
		{
			init();
	...
	template<class T, class Alloc>
	void KDeque<T, Alloc>::push_front(const value_type& val)
	{
		if (empty())
		{
			init();
	...
	template<class T, class Alloc>
	void KDeque<T, Alloc>::pop_front()
	{
		dataAllocator::destroy(beg_.cur_);
		++beg_;
	}
	template<class T, class Alloc>
	void KDeque<T, Alloc>::pop_back()
	{
		--end_;
		dataAllocator::destroy(end_.cur_);
	}
	template<class T, class Alloc>
	void KDeque<T, Alloc>::swap(KDeque<T, Alloc>& x)
	{
	...
	template <class T, class Alloc>
	bool operator== (const KDeque<T, Alloc>& lhs, const KDeque<T, Alloc>& rhs)
	{
	...
	template <class T, class Alloc>
	void swap(KDeque<T, Alloc>& x, KDeque<T, Alloc>& y)
	{
		x.swap(y);
	}
}
