...
namespace TinySTL{
	...
	KVector<T, Alloc>::~KVector()
	{
		destroyAndDeallocateAll();
	}
	template<class T, class Alloc>
	KVector<T, Alloc>::KVector(const size_type n)
	{
		allocateAndFillN(n, value_type());
	}
	template<class T, class Alloc>
	KVector<T, Alloc>::KVector(const size_type n, const value_type& value)
	{
		allocateAndFillN(n, value);
	}
	template<class T, class Alloc>
	template<class InputIterator>
	KVector<T, Alloc>::KVector(InputIterator first, InputIterator last)
	{
		//处理指针和数字间的区别的函数
		vector_aux(first, last, typename std::is_integral<InputIterator>::type());
	}
	template<class T, class Alloc>
	KVector<T, Alloc>::KVector(const KVector& v)
	{
		allocateAndCopy(v.start_, v.finish_);
	}
	template<class T, class Alloc>
	KVector<T, Alloc>::KVector(KVector&& v)
	{
		start_ = v.start_;
		finish_ = v.finish_;
	...
	template<class T, class Alloc>
	KVector<T, Alloc>& KVector<T, Alloc>::operator = (const KVector& v)
	{
		if (this != &v)
			allocateAndCopy(v.start_, v.finish_);
		return *this;
	...
	template<class T, class Alloc>
	KVector<T, Alloc>& KVector<T, Alloc>::operator = (KVector&& v)
	{
		if (this != &v)
		{
			destroyAndDeallocateAll();
	...
	//*************和容器的容量相关******************************
	template<class T, class Alloc>
	void KVector<T, Alloc>::resize(size_type n, value_type val = value_type())
	{
		if (n < size())
		{
	...
	template<class T, class Alloc>
	void KVector<T, Alloc>::reserve(size_type n)
	{
		if (n <= capacity())
			return;
	...
	//***************修改容器的相关操作**************************
	template<class T, class Alloc>
	typename KVector<T, Alloc>::iterator KVector<T, Alloc>::erase(iterator position)
	{
		return erase(position, position + 1);
	}
	template<class T, class Alloc>
	typename KVector<T, Alloc>::iterator KVector<T, Alloc>::erase(iterator first, iterator last)
	{
		//尾部残留对象数
	...
	template<class T, class Alloc>
	template<class InputIterator>
	void KVector<T, Alloc>::reallocateAndCopy(iterator position, InputIterator first, InputIterator last)
	{
	...
	template<class T, class Alloc>
	void KVector<T, Alloc>::reallocateAndFillN(iterator position, const size_type& n, const value_type& val)
	{
	...
	template<class T, class Alloc>
	template<class InputIterator>
	void KVector<T, Alloc>::insert_aux(iterator position,
		InputIterator first,
		InputIterator last,
		std::false_type){
	...
	void KVector<T, Alloc>::insert_aux(iterator position, Integer n, const value_type& value, std::true_type)
	{
		assert(n != 0);
		difference_type locationLeft = endOfStorage_ - finish_; // the size of left storage
	...
	template<class T, class Alloc>
	template<class InputIterator>
	void KVector<T, Alloc>::insert(iterator position, InputIterator first, InputIterator last)
	{
		insert_aux(position, first, last, typename std::is_integral<InputIterator>::type());
	}
	template<class T, class Alloc>
	void KVector<T, Alloc>::insert(iterator position, const size_type& n, const value_type& val)
	{
		insert_aux(position, n, val, typename std::is_integral<size_type>::type());
	}
	template<class T, class Alloc>
	typename KVector<T, Alloc>::iterator KVector<T, Alloc>::insert(iterator position, const value_type& val)
	{
	...
	template<class T, class Alloc>
	void KVector<T, Alloc>::push_back(const value_type& value)
	{
		insert(end(), value);
	}
	//***********逻辑比较操作相关*******************
	template<class T, class Alloc>
	bool KVector<T, Alloc>::operator == (const KVector& v)const
	{
		if (size() != v.size())
		{
			return false;
	...
	template<class T, class Alloc>
	bool operator == (const KVector<T, Alloc>& v1, const KVector<T, Alloc>& v2)
	{
		//return v1 == v2;
	...
	template<class T, class Alloc>
	void KVector<T, Alloc>::shrink_to_fit()
	{
	...
	template<class T, class Alloc>
	void KVector<T, Alloc>::clear()
	{
		dataAllocator::destroy(start_, finish_);
		finish_ = start_;
	}
	template<class T, class Alloc>
	void KVector<T, Alloc>::swap(KVector& v)
	{
		if (this != &v)
		{
	...
	template<class T, class Alloc>
	void KVector<T, Alloc>::pop_back()
	{
		--finish_;
		dataAllocator::destroy(finish_);
	}
	template<class T, class Alloc>
	void KVector<T, Alloc>::destroyAndDeallocateAll()
	{
		if (capacity() != 0)
		{
			dataAllocator::destroy(start_, finish_);
	...
	template<class T, class Alloc>
	void KVector<T, Alloc>::allocateAndFillN(const size_type n, const value_type& value)
	{
	...
	template<class T, class Alloc>
	template<class InputIterator>
	void KVector<T, Alloc>::allocateAndCopy(InputIterator first, InputIterator last)
	{
	...
	template<class T, class Alloc>
	template<class InputIterator>
	void KVector<T, Alloc>::vector_aux(InputIterator first, InputIterator last, std::false_type)
	{
		allocateAndCopy(first, last);
	}
	template<class T, class Alloc>
	template<class Integer>
	void KVector<T, Alloc>::vector_aux(Integer n, const value_type& value, std::true_type)
	{
		allocateAndFillN(n, value);
	}
	template<class T, class Alloc>
	typename KVector<T, Alloc>::size_type KVector<T, Alloc>::getNewCapacity(size_type len)const
	{
