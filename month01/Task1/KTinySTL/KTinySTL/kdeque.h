...
namespace TinySTL
{
	...
	namespace Detail
	{
		...
		private:
			...
			friend class ::TinySTL::KDeque;
		private:
			//typedef TinySTL::deque<T>* cntrPtr;
			typedef const ::TinySTL::KDeque<T>* cntrPtr;
			size_t mapIndex_;
			T *cur_;
			cntrPtr container_;
		public:
	...
	public:
		KDeque();
		explicit KDeque(size_type n, const value_type& val = value_type());
		template <class InputIterator>
		KDeque(InputIterator first, InputIterator last);
		KDeque(const KDeque& x);

		~KDeque();

		KDeque& operator= (const KDeque& x);
		KDeque& operator= (KDeque&& x);
	...
	public:
		template <class T, class Alloc>
		friend bool operator== (const KDeque<T, Alloc>& lhs, const KDeque<T, Alloc>& rhs);
		template <class T, class Alloc>
		friend bool operator!= (const KDeque<T, Alloc>& lhs, const KDeque<T, Alloc>& rhs);
		template <class T, class Alloc>
		friend void swap(KDeque<T, Alloc>& x, KDeque<T, Alloc>& y);
	};//end of deque
}

#include "Detail\kdeque_impl.h"
#endif