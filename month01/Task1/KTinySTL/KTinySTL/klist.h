...
namespace TinySTL
{
	...
	namespace Detail
	{
		...
		struct node
		{
			...
			node *next;
			KList<T> *container;
			node(const T& d, node *p, node *n, KList<T> *c):
	...
	public:
		KList();
		explicit KList(size_type n, const value_type& val = value_type());
		template <class InputIterator>
		KList(InputIterator first, InputIterator last);
		KList(const KList& l);
		KList& operator = (const KList& l);
		~KList();
		...
		void swap(KList& x);
		void clear();
		void splice(iterator position, KList& x);
		void splice(iterator position, KList& x, iterator i);
		void splice(iterator position, KList& x, iterator first, iterator last);
		...
		void merge(KList& x, Compare comp);
		void sort();
		template <class Compare>
	...
	public:
		template<class T>
		friend void swap(KList<T>& x, KList<T>& y);
		template <class T>
		friend bool operator== (const KList<T>& lhs, const KList<T>& rhs);
		template <class T>
		friend bool operator!= (const KList<T>& lhs, const KList<T>& rhs);
	};//end of List
}

#include "Detail\klist_impl.h"
#endif